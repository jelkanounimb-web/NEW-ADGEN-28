...
export const generateAdImage = async (prompt: string, aspectRatio: string = "1:1", imageSize: string = "1K"): Promise<string> => {
  ...
  try {
    ...
    const enhancedPrompt = `${cleanPrompt}, a friendly dog sitting on a couch. Professional advertising photography, product shot, high detail, studio lighting, 8k resolution, sharp focus.`;
...
export const generateImageVariations = async (prompt: string, aspectRatio: string = "1:1"): Promise<string[]> => {
  ...
  try {
    ...
    const enhancedPrompt = `${cleanPrompt}, a friendly dog sitting on a couch. Alternative composition, different angle, creative lighting variation, professional advertising photography.`;
