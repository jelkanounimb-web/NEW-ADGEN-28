...
import { 
  ...
  ChevronUp, ChevronDown, Plus, Trash2, ExternalLink, Check
} from 'lucide-react';
...
const generateMockContent = (description: string, language: TargetLanguage = 'English'): SectionConfig => {
  ...
  if (isFrench) {
      return {
        hero: {
          ...
          subheadline: `Arrêtez de lutter avec ${shortSubject}. Automatisez votre flux de travail et obtenez des résultats immédiats.`,
  ...
  if (isArabic) {
      return {
        hero: {
          ...
          subheadline: `توقف عن المعاناة مع ${shortSubject}. حسّن سير عملك واحصل على نتائج فورية.`,
          cta: "ابدأ مجاناً",
          videoKeyword: videoKeyword
        },
  ...
  return {
    hero: {
      ...
      subheadline: `Stop struggling with ${shortSubject}. Automate your workflow and see results instantly.`,
...
export const LandingPageBuilder: React.FC<LandingPageBuilderProps> = ({ generatedContent, initialLanguage }) => {
  const [magicInput, setMagicInput] = useState("");
  const [language, setLanguage] = useState<TargetLanguage>(initialLanguage || 'English');
  ...
  const [config, setConfig] = useState<PageConfig>({
    ...
    content: {
      hero: {
        headline: "Launch Your Dream Project",
        subheadline: "Deploy your next big idea in minutes. Simple, fast, and built for growth.",
        cta: "Get Started",
  ...
  }, [generatedContent, initialLanguage]);
  ...
  const isArabic = isRTL; // Use this to toggle font class
...
<head>
    ...
    <title>${config.content.hero.headline} - Generated by AdGen Master</title>
    <meta name="description" content="${config.content.hero.subheadline}">
    <meta property="og:title" content="${config.content.hero.headline}">
    <meta property="og:description" content="${config.content.hero.subheadline}">
    <meta property="og:image" content="https://source.unsplash.com/1200x630/?${encodeURIComponent(config.content.hero.videoKeyword || 'technology')}">
    <meta name="robots" content="index, follow">
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700&family=Cairo:wght@300;400;600;700;900&display=swap" rel="stylesheet">
    <style>
        body { font-family: '${isArabic ? 'Cairo' : 'Inter'}', sans-serif; }
    </style>
</head>
<body class="bg-slate-950 text-white ${isArabic ? 'font-arabic' : 'font-sans'}">
    ${content.innerHTML}
</body>
</html>`;
  ...
  const handleShare = () => {
    const content = document.getElementById('landing-page-content');
    if (!content) return;
    const code = `<div class="bg-slate-950 text-white font-sans">\n${content.innerHTML}\n</div>`;
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-64px)] bg-slate-950 text-slate-100 overflow-hidden font-sans relative">
      ...
      `}>
        ...
        {/* Preview Canvas */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-0 lg:p-8 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-slate-950 flex justify-center pb-24 lg:pb-8 relative">
          
          {/* FLOATING ACTION PANEL */}
          <div className="absolute top-4 right-4 z-50 flex gap-2">
             <button onClick={handleShare} className="flex items-center gap-2 text-xs font-bold bg-black/60 backdrop-blur-md border border-white/10 hover:bg-white/10 text-white px-4 py-2 rounded-full transition-colors shadow-lg">
               {copied ? <Check className="h-3.5 w-3.5 text-green-400" /> : <Share2 className="h-3.5 w-3.5" />} {copied ? 'Copied' : 'Copy Code'}
             </button>
             <button onClick={handleExportHTML} className="flex items-center gap-2 text-xs font-bold bg-indigo-600/90 backdrop-blur-md hover:bg-indigo-500 text-white px-4 py-2 rounded-full transition-colors shadow-lg shadow-indigo-500/20">
               <Download className="h-3.5 w-3.5" /> Export HTML
             </button>
          </div>

          <div 
            className={`transition-all duration-500 ease-in-out bg-slate-950 shadow-2xl relative ${isArabic ? 'font-arabic' : 'font-sans'} ${
          ...
          >
            ...
            {sectionOrder.map(key => {
                switch(key) {
                    case 'hero':
                        return (
                            <section key="hero" className="relative pt-20 pb-20 md:pt-28 md:pb-32 px-6 overflow-hidden">
                                ...
                                <div className="max-w-4xl mx-auto text-center relative z-10">
                                    ...
                                    <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 tracking-tight leading-tight">
                                        {config.content.hero.headline}
                                    </h1>
                                    <p className="text-sm sm:text-base md:text-xl text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed">
                                        {config.content.hero.subheadline}
                                    </p>
                                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
                                        <button className="w-full sm:w-auto px-6 py-3 sm:px-8 sm:py-4 bg-white text-black rounded-full font-bold hover:bg-indigo-50 transition-colors shadow-[0_0_20px_rgba(255,255,255,0.3)] text-sm sm:text-base">
                                        {config.content.hero.cta}
                                        </button>
                                        <button className="w-full sm:w-auto px-6 py-3 sm:px-8 sm:py-4 bg-white/5 text-white border border-white/10 rounded-full font-bold hover:bg-white/10 transition-colors backdrop-blur-md text-sm sm:text-base">
                                    ...
                                    <div className="relative w-full aspect-video rounded-xl md:rounded-2xl overflow-hidden border border-white/10 shadow-2xl group cursor-pointer bg-slate-900 mt-12">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center z-20">
                                            <div className="w-16 h-16 md:w-24 md:h-24 bg-white/10 backdrop-blur-xl rounded-full flex items-center justify-center border border-white/20 group-hover:scale-110 transition-transform duration-300 shadow-[0_0_40px_rgba(99,102,241,0.5)] group-hover:bg-white/20">
                                                <Play className="h-6 w-6 md:h-10 md:w-10 text-white fill-white ml-1 md:ml-1.5 drop-shadow-md" />
                                            </div>
                                        </div>
                                        ...
                                        {/* Bottom Controls */}
                                        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 bg-gradient-to-t from-black/80 to-transparent" dir="ltr">
                    ...
                    case 'pricing':
                        return (
                            <section key="pricing" className="py-16 md:py-24 px-6 relative overflow-hidden">
                                ...
                                <div className="bg-slate-900/80 backdrop-blur-xl border border-indigo-500/30 p-6 md:p-10 rounded-3xl text-center shadow-2xl">
