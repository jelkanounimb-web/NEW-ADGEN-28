...
import { Target, MessageSquare, Image as ImageIcon, Video, Hash, Copy, Check, Layout, Loader2, Sparkles, Download, Share2, Play, Film, RefreshCw, Hexagon, Smartphone, Monitor, Trash2, Globe, Facebook, AlertTriangle, X, ChevronDown, FileJson, Instagram, Youtube, Music2, Grid, Layers, Zap, Edit2, Volume2, Mic, Send, Scissors, Gauge, Type, Save, RotateCcw, Pause } from 'lucide-react';
...
const VideoEditor: React.FC<{ src: string; onSave: (newUrl: string) => void; onCancel: () => void }> = ({ src, onSave, onCancel }) => {
  ...
  useEffect(() => {
    ...
    if (video) {
      ...
      video.ontimeupdate = () => {
          if (!isProcessing) {
             // Loop preview if hitting end trim
  ...
  }, [endTime, startTime, isPlaying, isProcessing]);
  ...
  const handleProcess = () => {
      ...
      const processLoop = () => {
          ...
          // Ensure video plays for recording
          video.play(); 
          requestAnimationFrame(processLoop);
          
          const dur = endTime - startTime;
          if (dur > 0) {
            setProgress(((video.currentTime - startTime) / dur) * 100);
          }
      };
  ...
  return (
      <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row shadow-2xl">
              {/* Preview Area */}
              <div className="flex-1 bg-black relative flex items-center justify-center aspect-video md:aspect-auto">
                  ...
                  {isProcessing && (
                      <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center">
                          ...
                          <p className="text-white font-bold">Rendering... {Math.round(progress)}%</p>
                      </div>
                  )}
                  ...
                  {!isProcessing && (
                      <button 
                          onClick={togglePlay}
                          className="absolute inset-0 flex items-center justify-center bg-black/20 hover:bg-transparent transition-colors group"
                      >
                          {isPlaying ? 
                              <Pause className="h-12 w-12 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" /> : 
                              <Play className="h-12 w-12 text-white opacity-70 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
                          }
                      </button>
                  )}
              </div>
              ...
              {/* Controls */}
              <div className="w-full md:w-80 bg-slate-800 p-6 flex flex-col gap-6 border-l border-slate-700">
                  <div className="flex justify-between items-center border-b border-slate-700 pb-4">
                      <h3 className="text-white font-bold flex items-center gap-2"><Edit2 className="h-5 w-5"/> Edit Video</h3>
                  ...
                  <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2">
                          <Scissors className="h-3 w-3"/> Trim ({startTime.toFixed(1)}s - {endTime.toFixed(1)}s)
                      </label>
                      ...
                      <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-700">
                          {[0.5, 1, 1.5, 2].map(r => (
                              <button 
                                  key={r}
                                  onClick={() => setSpeed(r)}
                                  className={`flex-1 text-xs py-1.5 rounded ${speed === r ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                              >
                                  {r}x
                              </button>
                          ))}
                  ...
                  <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2">
                          <Type className="h-3 w-3"/> Overlay Text
                      </label>
                      <input 
                          type="text" 
                          value={text}
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-700 flex flex-col gap-2">
                      ...
                      <button 
                          ...
                          className="w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-2 rounded-lg flex items-center justify-center gap-2 text-sm"
                      >
...
export const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ data }) => {
  ...
  const handleAPIError = (e: any) => {
      ...
      if (msg.includes("404") || msg.includes("Requested entity was not found") || msg.includes("API Key")) {
  ...
  const handleGenerateLogo = async () => {
    ...
    try {
      ...
      let logoPrompt = `MUST FOLLOW STYLE/COLOR: ${logoStylePreference}. Professional logo design for: ${data.strategy.usp}. `;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-fade-in relative">
      ...
      {/* Video Editor Modal */}
      {showVideoEditor && adVideo && (
          <VideoEditor 
            src={adVideo} 
            onSave={(newUrl) => { setAdVideo(newUrl); setShowVideoEditor(false); }} 
      ...
      <div id="live-preview" className="lg:col-span-3 mt-6 border-t border-slate-700 pt-8">
         <div className="flex justify-center gap-4 mb-6 relative">
             ...
             <div className="absolute top-0 left-4">
                  <button 
                    ...
                    className="flex items-center gap-2 bg-slate-800 border border-slate-600 text-white px-3 py-2 rounded-lg text-xs font-bold shadow-lg hover:bg-slate-700 transition-all"
                  >
                      {activeTab === 'IMAGE' ? <Film className="h-4 w-4 text-indigo-400" /> : <ImageIcon className="h-4 w-4 text-emerald-400" />}
                      Switch to {activeTab === 'IMAGE' ? 'Video' : 'Image'}
                  </button>
             </div>
         </div>
         ...
         <div className="flex justify-center">
             <div className="w-full max-w-md bg-white rounded-lg overflow-hidden shadow-2xl ring-1 ring-black/10">
                 ...
                 {/* Media Area */}
                 <div className="aspect-square bg-slate-100 relative flex items-center justify-center group overflow-hidden">
                     {activeTab === 'IMAGE' && (
                         adImage ? (
                             <>
                     ...
                     {activeTab === 'VIDEO' && (
                         adVideo ? (
                            <>
                                <video src={adVideo} controls autoPlay muted loop className="w-full h-full object-contain bg-black"/>
                                <button 
                                    onClick={() => setShowVideoEditor(true)}
                                    className="absolute top-2 right-2 bg-black/70 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-600 transition-colors flex items-center gap-1 shadow-lg backdrop-blur-sm"
                                >
                                    <Edit2 className="h-3 w-3" /> Edit Video
                                </button>
                            </>
                         ) : 
